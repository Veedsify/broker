<nav class="fixed top-0 left-0 right-0 z-10 flex items-center bg-[#2c3b1e]   print:hidden  ltr:pr-6 rtl:pl-6">

    <div class="flex justify-between w-full">
        <div class="flex items-center topbar-brand">
            <div
                class="hidden lg:flex navbar-brand items-center justify-between shrink px-6 h-[70px]  ltr:border-r rtl:border-l bg-[#2c3b1e] border-gray-50   shadow-none">
                <a href="/"
                    class="flex items-center text-lg flex-shrink-0 font-bold dark:text-white leading-[69px]">
                    <img src="/assets/images/logo2.png" alt="" width="150px">
                </a>
            </div>
            <button type="button"
                class="group-data-[sidebar-size=sm]:border-b border-b border-[#2c3b1e]  dark:lg:border-transparent lg:border-transparent  group-data-[sidebar-size=sm]:border-[#627e19] group-data-[sidebar-size=sm]: text-gray-100 dark:text-white h-[70px] px-4 ltr:-ml-[52px] rtl:-mr-14 py-1 vertical-menu-btn text-16"
                id="vertical-menu-btn">
                <i class="fa fa-fw fa-bars"></i>
            </button>

        </div>
        <div class="flex justify-between w-full items-center border-b border-[#627e19]  ltr:pl-6 rtl:pr-6">
            <div>
                <form class="hidden app-search xl:block">
                    <div class="relative inline-block">
                        <input type="text"
                            class="pl-4 pr-[40px] border-0 placeholder:text-gray-100 rounded text-white bg-[#627e19] text-13 placeholder:text-13  max-w-[223px]"
                            placeholder="Search...">
                        <button
                            class="py-1.5 px-2.5 w-9 h-[34px] text-white bg-[#bce70c] inline-block absolute ltr:right-1 top-1 rounded shadow "
                            type="button"><i class="align-middle bx bx-search-alt"></i></button>
                    </div>
                </form>
            </div>
            <div class="flex">
                <div>
                    <div class="relative block dropdown sm:hidden">
                        <button type="button" class="text-xl px-4 h-[70px] text-white dropdown-toggle"
                            data-dropdown-toggle="navNotifications">
                            <i data-feather="search" class="w-5 h-5"></i>
                        </button>

                        <div class="absolute top-0 z-50 hidden px-4 mx-4 list-none bg-bgprimary border rounded shadow  dropdown-menu -left-36 w-72 border-gray-50   dark:text-gray-300"
                            id="navNotifications">
                            <form class="py-3 dropdown-item" aria-labelledby="navNotifications">
                                <div class="m-0 form-group">
                                    <div class="flex w-full">
                                        <input type="text" class="border-gray-100   w-fit" placeholder="Search ..."
                                            aria-label="Search Result">
                                        <button
                                            class="text-white border-l-0 border-transparent rounded-l-none btn btn-primary bg-green-500"
                                            type="submit"><i class="mdi mdi-magnify"></i></button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="relative hidden dropdown language sm:block">
                    <button class="btn border-0 py-0 dropdown-toggle px-3 h-[70px]" type="button" aria-expanded="false"
                        data-dropdown-toggle="navNotifications">
                        <img src="data:image/jpeg;base64,"
                            alt="" class="h-4" id="header-lang-img">
                    </button>
                    <div class="absolute z-50 hidden w-40 list-none bg-bgprimary rounded shadow dropdown-menu -left-24 "
                        id="navNotifications">
                        <ul class="border border-gray-50 dark:border-gray-700" aria-labelledby="navNotifications">
                            <li>
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-gray-100 hover:bg-gray-50/50 dark:text-gray-200 dark:hover:bg-zinc-600/50 "><img
                                        src="data:image/jpeg;base64,"
                                        alt="user-image" class="inline-block h-3 mr-1"> <span
                                        class="align-middle">English</span></a>
                            </li>
                            <li>
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-gray-100 hover:bg-gray-50/50 dark:text-gray-200 dark:hover:bg-zinc-600/50 "><img
                                        src="data:image/jpeg;base64,"
                                        alt="user-image" class="inline-block h-3 mr-1"> <span
                                        class="align-middle">Spanish</span></a>
                            </li>
                            <li>
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-gray-100 hover:bg-gray-50/50 dark:text-gray-200 dark:hover:bg-zinc-600/50 "><img
                                        src="data:image/jpeg;base64,"
                                        alt="user-image" class="inline-block h-3 mr-1"> <span
                                        class="align-middle">German</span></a>
                            </li>
                            <li>
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-gray-100 hover:bg-gray-50/50 dark:text-gray-200 dark:hover:bg-zinc-600/50 "><img
                                        src="data:image/jpeg;base64,"
                                        alt="user-image" class="inline-block h-3 mr-1"> <span
                                        class="align-middle">Italian</span></a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div>
                    <div class="relative dropdown text-white">
                        <div class="relative">
                            <button type="button" class="btn border-0 h-[70px] dropdown-toggle px-4  ltr:mr-2 rtl:ml-2"
                                aria-expanded="false" data-dropdown-toggle="notification">
                                <i data-feather="bell" class="w-5 h-5"></i>
                            </button>
                            <span
                                class="absolute text-xs px-1 bg-red-500 text-white font-medium rounded-full left-6 top-2.5">
                                {{ $activities->count() }}
                            </span>
                        </div>
                        <div class="absolute z-50 hidden list-none bg-bgsecondary rounded shadow dropdown-menu w-80  "
                            id="notification">
                            <div class="border rounded border-[#627e19]" aria-labelledby="notification">
                                <div class="grid grid-cols-12 p-4">
                                    <div class="col-span-6">
                                        <h6 class="m-0 text-gray-100 "> Notifications </h6>
                                    </div>
                                </div>
                                <div class="max-h-56" data-simplebar="">
                                    <div>
                                        @foreach ($activities as $activity)
                                            <a href="#!" class="text-reset notification-item">
                                                <div
                                                    class="flex px-4 py-2 hover:bg-gray-50/50 dark:hover:bg-zinc-700/50">
                                                    <div class="flex-grow">
                                                        <h6 class="mb-1 text-gray-100 ">{{ $activity->name }}
                                                        </h6>
                                                        <div class="text-white text-13">
                                                            <p class="mb-1 dark:text-gray-400">
                                                                {{ $activity->description }}</p>
                                                            <p class="mb-0"><i
                                                                    class="mdi mdi-clock-outline dark:text-gray-400"></i>
                                                                <span>
                                                                    {{ $activity->created_at->diffForHumans() }}
                                                                </span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="grid p-1 border-t border-gray-50  justify-items-center">
                                    <a class="border-0 btn text-lime-500" href="javascript:void(0)">
                                        <i class="mr-1 mdi mdi-arrow-right-circle"></i> <span>View More..</span>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="relative dropdown">
                        <button type="button"
                            class="flex items-center px-3 py-2 h-[70px] border-x border-bgborder bg-bgsecondary/70 dropdown-toggle text-gray-100 "
                            id="page-header-user-dropdown" data-bs-toggle="dropdown" aria-haspopup="true"
                            aria-expanded="true">
                            <img class="border-[3px] border-bgborder rounded-full w-9 h-9 ltr:xl:mr-2 rtl:xl:ml-2"
                                src="{{ auth()->user()->avatar }}" alt="Header Avatar">
                            <span class="hidden  font-medium xl:block">{{ explode(' ', auth()->user()->name)[0] }}
                                {{ explode(' ', auth()->user()->name)[1][0] }}.</span>
                            </span>
                            <i class="hidden align-bottom mdi mdi-chevron-down xl:block"></i>
                        </button>
                        <div class="absolute top-0 z-50 hidden w-40 list-none bg-bgprimary dropdown-menu dropdown-animation rounded shadow text-gray-100 "
                            id="profile/log">
                            <div class="border border-bgborder " aria-labelledby="navNotifications">
                                <div class="dropdown-item ">
                                    <a class="block px-3 py-2 hover:bg-gray-50/50"
                                        href="{{ route('account.profile') }}">
                                        <i class="mr-1 align-middle mdi mdi-face-man text-16"></i> Profile
                                    </a>
                                </div>

                                <hr class="border-bgborder">
                                <div class="dropdown-item ">
                                    <form action="{{ route('logout') }}" method="POST">
                                        @csrf
                                        <button class="block p-3 hover:bg-gray-50/50w-full">
                                            <i class="mr-1 align-middle mdi mdi-logout text-16"></i> Logout
                                        </button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</nav>
